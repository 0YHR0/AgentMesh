...
| `credential_broker` | `persistent_identity`, `policy_approval`, `a2a_federation` | Workload-bound SecretReference, binding and lease audit APIs |
...
The `credential_broker` Gate depends on `persistent_identity`, `policy_approval`, and
`a2a_federation`, and is explicit opt-in. It enables metadata-only SecretReferences,
Permit-bound workload CredentialBindings, short-lived lease evidence, and authenticated A2A
Bearer injection. Startup additionally requires an active tenant `SERVICE` Principal UUID through
