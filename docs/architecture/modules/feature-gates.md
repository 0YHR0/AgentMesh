...
| Profile | Registry management | Deployment management | Artifact Service | Intended use |
|---|---:|---:|---:|---|
| `minimal` (default) | Off | Off | Off | 第一次运行、核心 Task API、单内置 Agent |
| `standard` | On | Off | Off | 自定义 Agent、版本、能力和候选发现 |
| `full` | On | On | On | 完整的当前正式功能集合 |
