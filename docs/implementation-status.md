...
| Web Console | Partial | Built-in zero-build Console for Task operations, Agent-aware role binding, native SVG Mission Map with Agent stations, DAG routes, durable dispatch/output pulses, governed Handoff/MCP/A2A/Policy/Plan Patch routes, external Tool/peer/gate/patch nodes, animated persisted interaction packets, session-preserved transport/Agent/status/kind/trace filters, deterministic event-time replay with pause/live/step controls and projected Run/Subtask state, browser-local replay bookmarks, sanitized `agentmesh.mission-replay.v1` JSON export, bounded zoom/pan/fit/reset/focus camera navigation with clickable overview minimap, keyboard-selectable inspector and unified Event Deck, retained work-card fallback, Run history/output, feature-gated candidate Plan JSON editing with verifier evidence and atomic apply, feature-aware Agent authoring/review/publication, exact publish-intent requests, Policy approval/Permit handoff, immutable Artifact creation/version catalog with authenticated preview/download and Run lineage, immutable runtime-policy catalog, governed MCP invocation and cross-domain Task activity timelines, feature-gated realtime refresh with polling fallback, optional session Bearer token, and an opt-in deterministic research-brief showcase | Semantic clustering/virtualization for very large graphs, server-persisted shared bookmarks and advanced operations UI |
...
2. Add semantic clustering/virtualization beyond the implemented large-graph camera navigation,
   and optional server-shared replay bookmarks only when operators need to collaborate on
   high-volume Task histories.
