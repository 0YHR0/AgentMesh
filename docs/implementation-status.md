...
| Web Console | Partial | Built-in zero-build Console for Task creation/list/detail, Agent-aware role binding, coordinated dependency visualization, Run assignment/history, output inspection, feature-aware immutable Agent/version runtime-policy catalog, Task-scoped governed MCP invocation timeline, polling, optional session Bearer token, and run/pause/resume/cancel controls | Agent/version authoring and publishing, SSE, scalable graph layout, approval inbox, artifacts, cross-domain audit timeline and advanced operations UI |
...
1. Expand the Console with Agent/version authoring and publishing, realtime events, approval inbox,
   artifacts, and a cross-domain audit timeline only as their reference workflow requires them.
